...
)  # BoundingBoxPredictor is the custom CNN
from bounding_box_yunet import YuNetFaceDetector


"""
Logging configuration for the face recognition application. This setup directs logs to both a file and the console
with a specified format that includes the timestamp, log level, and message.

- Log Level: INFO
- Log Format: "[timestamp] - [log level] - [message]"
- Handlers:
  - FileHandler: Writes logs to 'face_recognition.log'
...
"""
The PersonDatabase class is responsible for managing person data and embeddings storage in an SQLite database.
...
lock : threading.Lock
    A lock object for thread-safe database access.

Methods
-------
_create_table()
    Creates the 'persons' table if it doesn't exist.
insert_person(person_id, name, embedding)
    Inserts a new person record into the database.
get_embedding(person_id)
    Retrieves the stored embedding vector for a person.
get_persons()
    Retrieves a list of all registered persons from the database.
"""
...
class PersonDatabase:
    ...
    def add_person(self, embedding, name, is_allowed):
        ...
        with self.lock:
            person_id = datetime.now().strftime(
                "%Y%m%d_%H%M%S"
    ...
    def find_match(self, embedding, threshold=0.3):
        """
        Finds a match in the database with a similarity above the given threshold.
        Args:
            embedding (numpy.ndarray): The embedding to search for.
            threshold (float, optional): The similarity threshold. Defaults to 0.3.
        Returns:
            tuple: The ID, name, and is_allowed of the best match, or None if none found.
        """
        with self.lock:
            cursor = self.conn.execute(
                "SELECT id, name, embedding, is_allowed FROM persons"
            )
            ...
            for row in cursor:
                ...
                logging.debug(
                    f"Distance to {row[1]}: {distance:.5f}"
                )  # Use debug for frequent logs
            ...
            if min_distance < threshold:
                ...
                logging.info(
                    f"Match found: {best_match[1]} with distance {min_distance:.5f} (Threshold: {threshold:.3f})"
                )
                return (
                    best_match[0],
                    {"name": best_match[1], "is_allowed": bool(best_match[3])},
                    min_distance,
                )

            # No match found within the threshold
            logging.info(
                f"No match found. Closest was {min_distance:.5f} (Threshold: {threshold:.3f})"
            )
    ...
    def get_all_persons(self):
        ...
        with self.lock:
            cursor = self.conn.execute(
                "SELECT id, name, is_allowed, created_at FROM persons ORDER BY created_at DESC"
            )
            persons = []
            for row in cursor:
                persons.append(
    ...
    person_db : PersonDatabase
        The database object for storing and retrieving person data and embeddings.
    embedding_predictor : EmbeddingPredictor
        The object for generating embeddings from face images.
    cap : cv2.VideoCapture
        The OpenCV VideoCapture object for built-in camera.
    built_in_camera_index : int
        The index of the built-in camera to use (default: 0).
    ip_cam_url : str
        The URL of the IP camera stream (default: "http://192.168.1.5/cam-hi.jpg").
    active_camera_type : str
        The type of camera currently in use ("Built-in Cam" or "IP Cam").
    yunet_model_path : str
        The file path to the YuNet face detector model (default: "models/bbox_models/YuNet/face_detection_yunet_2023mar.onnx").
    haar_cascade_path : str
        The file path to the Haar cascade face detector model (default: OpenCV's default).
    custom_cnn_model_path : str
        The file path to the custom CNN face detector model (default: "models/bbox_models/v5/bbox_v5_randomly_augmented_epoch_3.pth").
    threshold : float
        The similarity threshold for face recognition (default: 0.25).
    num_samples : int
        The number of samples to take for face recognition (default: 5).
    """
...
class FaceRecognitionApp:
    ...
    def __init__(
        ...
        initial_ip_cam_url="http://192.168.1.5/cam-hi.jpg",
        ...
        self.bbox_predictor = (
            None  # Initialize as None, will be set by _initialize_bbox_predictor
        )
        ...
        self.processing = (
            False  # Flag to prevent concurrent registration/login operations
        )
        self.threshold = threshold  # Cosine distance threshold for recognition
        self.num_samples = (
            num_samples  # Number of face samples to average for robust embedding
        )
        self.embedding_queue = Queue(
            maxsize=self.num_samples
        )  # Queue for storing recent embeddings
        self.lock = (
            threading.Lock()
        )  # Lock for thread-safe access to shared resources (e.g., queue)
        ...
        self.video_label = ctk.CTkLabel(
            self.video_frame, text="Video Feed", anchor="center"
        )
        ...
        ctk.CTkLabel(
            self.reg_frame, text="Registration", font=("Arial", 18, "bold")
        ).pack(padx=5, pady=5)
        ...
        ctk.CTkCheckBox(
            self.reg_frame, text="Allowed Access", variable=self.is_allowed_var
        ).pack(padx=5, pady=2)
        self.register_btn = ctk.CTkButton(
            self.reg_frame, text="Register Person", command=self.register_person
        )
        ...
        ctk.CTkLabel(self.login_frame, text="Login", font=("Arial", 18, "bold")).pack(
            padx=5, pady=5
        )
        self.login_btn = ctk.CTkButton(
            self.login_frame, text="Login Person", command=self.login_person
        )
        ...
        self.threshold_label = ctk.CTkLabel(
            self.threshold_frame, text=f"Current Threshold: {self.threshold:.3f}"
        )
        self.threshold_label.pack(padx=5, pady=5)
        self.min_distance_label = ctk.CTkLabel(
            self.threshold_frame, text="Last Match Distance: N/A"
        )
        ...
        ctk.CTkLabel(
            self.results_logs_section_frame,
            text="Application Results & Logs",
            font=("Arial", 18, "bold"),
        ).pack(padx=5, pady=5)
        ...
        self.results_text = ctk.CTkTextbox(
            self.results_logs_section_frame, height=15
        )  # Adjusted height
        ...
        ctk.CTkLabel(self.camera_settings_frame, text="Camera Source:").pack(
            padx=5, pady=2
        )
        ...
        ctk.CTkLabel(self.camera_settings_frame, text="IP Cam URL:").pack(
            padx=5, pady=2
        )
        ...
        self.apply_camera_settings_btn = ctk.CTkButton(
            self.camera_settings_frame,
        ...
        ctk.CTkLabel(self.bbox_model_settings_frame, text="Model Type:").pack(
            padx=5, pady=2
        )
        # Added "Custom CNN Detector"
        self.bbox_model_options = [
            "YuNet Detector",
        ...
        self.apply_bbox_model_btn = ctk.CTkButton(
            self.bbox_model_settings_frame,
            text="Apply Model Settings",
            command=self.apply_bbox_model_settings,
        )
        ...
        self.manage_persons_content_frame.pack(
            fill="both", expand=True, padx=10, pady=10
        )
        ctk.CTkLabel(
            self.manage_persons_content_frame,
        ...
        self.person_list_header_frame.grid_columnconfigure(
            0, weight=1, minsize=100
        )  # ID
        self.person_list_header_frame.grid_columnconfigure(
            1, weight=2, minsize=150
        )  # Name
        self.person_list_header_frame.grid_columnconfigure(
            2, weight=1, minsize=80
        )  # Allowed
        self.person_list_header_frame.grid_columnconfigure(
            3, weight=2, minsize=180
        )  # Registered At
        self.person_list_header_frame.grid_columnconfigure(
            4, weight=2, minsize=120
        )  # Action (increased minsize/weight)

        ctk.CTkLabel(
            self.person_list_header_frame, text="ID", font=("Arial", 12, "bold")
        ).grid(row=0, column=0, padx=2, pady=2, sticky="w")
        ctk.CTkLabel(
            self.person_list_header_frame, text="Name", font=("Arial", 12, "bold")
        ).grid(row=0, column=1, padx=2, pady=2, sticky="w")
        ctk.CTkLabel(
            self.person_list_header_frame, text="Allowed", font=("Arial", 12, "bold")
        ).grid(row=0, column=2, padx=2, pady=2, sticky="w")
        ctk.CTkLabel(
            self.person_list_header_frame,
            text="Registered At",
            font=("Arial", 12, "bold"),
        ).grid(row=0, column=3, padx=2, pady=2, sticky="w")
        ctk.CTkLabel(
            self.person_list_header_frame, text="Action", font=("Arial", 12, "bold")
        ).grid(row=0, column=4, padx=2, pady=2, sticky="w")

        # Use a CTkCanvas with scrollbars for the main list area
        self.person_list_canvas_container = ctk.CTkFrame(
            self.manage_persons_content_frame
        )
        self.person_list_canvas_container.pack(
        ...
        try:
            ...
            bg_color_canvas = ctk.ThemeManager.theme["CTkFrame"]["fg_color"][mode_index]
        ...
        self.person_list_v_scrollbar = ctk.CTkScrollbar(
            self.person_list_canvas_container,
            orientation="vertical",
            command=self.person_list_canvas.yview,
        )
        self.person_list_v_scrollbar.grid(row=0, column=1, sticky="ns")
        self.person_list_canvas.configure(
            yscrollcommand=self.person_list_v_scrollbar.set
        )

        self.person_list_h_scrollbar = ctk.CTkScrollbar(
            self.person_list_canvas_container,
            orientation="horizontal",
            command=self.person_list_canvas.xview,
        )
        self.person_list_h_scrollbar.grid(row=1, column=0, sticky="ew")
        self.person_list_canvas.configure(
            xscrollcommand=self.person_list_h_scrollbar.set
        )

        self.person_list_inner_frame = ctk.CTkFrame(
            self.person_list_canvas, fg_color="transparent"
        )
        # create_window places the frame inside the canvas
        self.person_list_canvas.create_window(
            (0, 0), window=self.person_list_inner_frame, anchor="nw", tags="inner_frame"
        )
        ...
        self.refresh_persons_btn = ctk.CTkButton(
            self.manage_persons_content_frame,
            text="Refresh Person List",
            command=self._load_persons_to_gui,
        )
        ...
        self.exit_btn = ctk.CTkButton(
            self.control_panel_frame, text="Exit Program", command=self.exit_program
        )
        self.exit_btn.pack(
            fill="x", padx=10, pady=10, side="bottom"
        )  # Placed at the bottom of the control panel frame
    ...
    def _setup_gui_logger(self):
        ...
        class GUILogHandler(logging.Handler):
            ...
            def emit(self, record):
                ...
                self.textbox.after(
                    0, lambda: self.textbox.see(ctk.END)
    ...
    def on_camera_type_selected(self, choice):
        ...
        self.status_label.configure(
            text=f"Please click 'Apply Camera Settings' to switch to {self.active_camera_type}.",
            text_color="blue",
        )
    ...
    def apply_camera_settings(self):
        ...
        if self.active_camera_type == "IP Cam" and not new_ip_cam_url:
            messagebox.showerror(
                "Configuration Error", "IP Cam URL cannot be empty when selected."
            )
            self.status_label.configure(
                text="Error: IP Cam URL is empty.", text_color="red"
            )
            # Revert to previous camera type if invalid input
            self.camera_type_var.set(
                "Built-in Cam" if self.active_camera_type == "IP Cam" else "IP Cam"
            )
        ...
        logging.info(
            f"Applying camera settings: type={self.active_camera_type}, URL={self.ip_cam_url}"
        )
    ...
    def on_bbox_model_type_selected(self, choice):
        ...
        self.status_label.configure(
            text=f"Please click 'Apply Model Settings' to switch to {self.active_bbox_model_type}.",
            text_color="blue",
        )
    ...
    def apply_bbox_model_settings(self):
        ...
        logging.info(
            f"Applying BBox model settings: type={self.active_bbox_model_type}"
        )
        self._initialize_bbox_predictor()
        # After re-initializing, it's good to provide feedback to the user
        if self.bbox_predictor:
            self.status_label.configure(
                text=f"{self.active_bbox_model_type} applied successfully.",
                text_color="green",
            )
        else:
            self.status_label.configure(
                text=f"Failed to apply {self.active_bbox_model_type}.", text_color="red"
            )
    ...
    def _initialize_bbox_predictor(self):
        ...
        if self.active_bbox_model_type == "YuNet Detector":
            try:
                ...
                logging.error(
                    f"Error initializing YuNetFaceDetector: {e}", exc_info=True
                )
                messagebox.showerror(
                    "Model Error",
                    f"Failed to load YuNet Detector: {e}. Check model path and ensure 'models/bbox_models/YuNet/face_detection_yunet_2023mar.onnx' exists.",
                )
                self.status_label.configure(
                    text="Error loading YuNet Detector.", text_color="red"
                )
            ...
            try:
                ...
                self.bbox_predictor = BoundingBoxDetector(
                    cascade_path=self.haar_cascade_path
                )
                logging.info("BoundingBoxDetector (Haar Cascade) initialized.")
            except Exception as e:
                logging.error(
                    f"Error initializing BoundingBoxDetector (Haar Cascade): {e}",
                    exc_info=True,
                )
                messagebox.showerror(
                    "Model Error",
                    f"Failed to load Haar Cascade Detector: {e}. Check cascade path and ensure '{self.haar_cascade_path}' exists.",
                )
                self.status_label.configure(
                    text="Error loading Haar Cascade Detector.", text_color="red"
                )
                self.bbox_predictor = None  # Ensure predictor is None on failure
            ...
            try:
                ...
                self.bbox_predictor = BoundingBoxPredictor(
                    model_path=self.custom_cnn_model_path, device="cpu"
                )
                logging.info("Custom CNN BoundingBoxPredictor initialized.")
            except Exception as e:
                logging.error(
                    f"Error initializing Custom CNN BoundingBoxPredictor: {e}",
                    exc_info=True,
                )
                messagebox.showerror(
                    "Model Error",
                    f"Failed to load Custom CNN Detector: {e}. Check model path and ensure '{self.custom_cnn_model_path}' exists.",
                )
                self.status_label.configure(
                    text="Error loading Custom CNN Detector.", text_color="red"
                )
                self.bbox_predictor = None  # Ensure predictor is None on failure
        else:
            logging.error(f"Unknown BBox model type: {self.active_bbox_model_type}")
            messagebox.showerror(
                "Model Error", f"Unknown BBox model type: {self.active_bbox_model_type}"
            )
            self.status_label.configure(
                text="Unknown BBox model type.", text_color="red"
            )
    ...
    def extract_face(self, frame):
        ...
        if self.bbox_predictor is None:
            ...
            self.root.after(
                ...
                lambda: self.status_label.configure(
                    text="No face detector loaded. Select a model.", text_color="orange"
                ),
            )
            return None
        ...
        if len(faces) > 0:
            ...
            embedding = self.embedding_predictor.generate_embedding(
                cropped_face
            ).flatten()
    ...
    def capture_multiple_embeddings(self, num_samples=10):
        ...
        self.status_label.configure(
            text="Detecting face and collecting samples...", text_color="yellow"
        )
        ...
        while (
            time.time() - initial_check_time < 2
        ):  # Wait up to 2 seconds for initial detection
        ...
        if not face_detected_initially:
            self.status_label.configure(
                text="No face detected in initial scan.", text_color="red"
            )
            return None
        ...
        while self.embedding_queue.qsize() < num_samples:
            if time.time() - start_time > timeout:
                if self.embedding_queue.qsize() == 0:
                    self.status_label.configure(
                        text="Failed to collect any samples after initial detection.",
                        text_color="red",
                    )
                    return None
                # If we have *some* samples but not `num_samples` due to timeout, take what we have
                self.status_label.configure(
                    text=f"Timeout: Collected {self.embedding_queue.qsize()} of {num_samples} samples.",
                    text_color="orange",
                )
        ...
        if embeddings:
            self.status_label.configure(
                text="Samples collected successfully.", text_color="green"
            )
        ...
        self.status_label.configure(
            text="No samples collected (unexpected error).", text_color="red"
        )
        return None
    ...
    def _start_camera_capture(self):
        ...
        if self.active_camera_type == "Built-in Cam":
            ...
            if not self.cap.isOpened():
                logging.error(
                    f"Failed to open built-in camera with index {self.built_in_camera_index}"
                )
                messagebox.showerror(
                    "Camera Error",
                    f"Could not open built-in camera {self.built_in_camera_index}. Please ensure it's connected and not in use.",
                )
                self.status_label.configure(
                    text="Failed to open built-in camera.", text_color="red"
                )
                self.running = False
                return
            logging.info(
                f"Started built-in camera capture (index: {self.built_in_camera_index})."
            )
            self.status_label.configure(
                text="Built-in camera connected.", text_color="green"
            )
        elif self.active_camera_type == "IP Cam":
            if not self.ip_cam_url:
                messagebox.showerror("Configuration Error", "IP Cam URL is not set.")
                self.status_label.configure(
                    text="IP Cam URL not set.", text_color="red"
                )
                self.running = False
                return
            logging.info(f"Attempting to connect to IP Cam at URL: {self.ip_cam_url}")
            self.status_label.configure(
                text=f"Connecting to IP Cam: {self.ip_cam_url}...", text_color="yellow"
            )
            ...
            self.status_label.configure(
                text="Unknown camera type selected.", text_color="red"
            )
            self.running = False
            return
        ...
        if (
            self.running
        ):  # Check self.running as it might have been set to False by error above
            self.capture_thread = threading.Thread(target=self._capture_frames_loop)
            self.capture_thread.daemon = (
                True  # Allows thread to exit when main app exits
            )
    ...
    def _stop_camera_capture(self):
        ...
        if self.capture_thread and self.capture_thread.is_alive():
            ...
            if self.capture_thread.is_alive():
                logging.warning(
                    "Capture thread did not terminate cleanly within timeout."
                )
        ...
        while self.running:
            ...
            if self.active_camera_type == "Built-in Cam":
                if self.cap and self.cap.isOpened():
                    ...
                    if not ret:
                        logging.error(
                            "Failed to grab frame from built-in camera. Attempting to re-acquire..."
                        )
                        self.cap.release()
                        time.sleep(1)
                        self.cap = cv2.VideoCapture(self.built_in_camera_index)
                        if not self.cap.isOpened():
                            logging.error(
                                "Failed to re-acquire built-in camera. Stopping capture."
                            )
                            ...
                            self.root.after(
                                ...
                                lambda: messagebox.showerror(
                                    "Camera Error",
                                    "Built-in camera disconnected or inaccessible. Please restart the application.",
                                ),
                            )
                            self.root.after(
                                0,
                                lambda: self.status_label.configure(
                                    text="Built-in camera lost.", text_color="red"
                                ),
                            )
                    ...
                    logging.error(
                        "Built-in camera object not initialized or closed unexpectedly."
                    )
                    self.running = False
                    self.root.after(
                        0,
                        lambda: messagebox.showerror(
                            "Camera Error",
                            "Built-in camera stream lost. Please try restarting the application.",
                        ),
                    )
                    self.root.after(
                        0,
                        lambda: self.status_label.configure(
                            text="Built-in camera stream lost.", text_color="red"
                        ),
                    )
                    continue
                ...
                try:
                    ...
                    self.root.after(
                        0,
                        lambda: self.status_label.configure(
                            text="IP Cam connected.", text_color="green"
                        ),
                    )
                    ...
                    self.root.after(
                        0,
                        lambda: self.status_label.configure(
                            text=f"IP Cam error: {e}. Retrying...", text_color="orange"
                        ),
                    )
                    ...
                    logging.error(
                        f"Error fetching/decoding IP Cam frame: {e}. Retrying..."
                    )
                    self.root.after(
                        0,
                        lambda: self.status_label.configure(
                            text=f"IP Cam decode error: {e}. Retrying...",
                            text_color="orange",
                        ),
                    )
                    time.sleep(1)  # Wait before retry
            ...
            try:
                ...
                logging.error(
                    f"Frame processing error: {e}", exc_info=True
                )  # Log full traceback
    ...
    def _update_frame_on_gui(self):
            ...
            if label_width > 1 and label_height > 1:  # Check for minimal size
                ...
                if img_width > 0 and img_height > 0:
                    ...
                    try:
                        ...
                        resized_frame = self.current_frame.resize(
                        ...
                        logging.error(
                            f"Error during image resize: {e}. Original: {img_width}x{img_height}, Target: {new_width}x{new_height}"
                        )
                else:
                    logging.warning(
                        f"Skipping frame update: current_frame has invalid dimensions {img_width}x{img_height}"
                    )
        ...
        if self.running:
            self.root.after(
                30, self._update_frame_on_gui
            )  # Schedule next frame update (approx 33 FPS)
    ...
    def register_person(self):
        ...
        if self.processing:
            messagebox.showwarning(
                "Processing",
                "Please wait, a registration or login operation is already in progress.",
            )
            return
        ...
        if not name:
            messagebox.showerror(
                "Input Error", "Please enter a name for the person before registering."
            )
            return
        ...
        def register_thread_target():
            ...
            try:
                ...
                self.status_label.configure(
                    text="Registering: Capturing face samples...", text_color="yellow"
                )
                ...
                if embedding is None:
                    messagebox.showerror(
                        "Registration Failed",
                        "Could not extract face features. Please ensure your face is well-lit, clearly visible, and held still within the video frame during capture.",
                    )
                    self.status_label.configure(
                        text="Registration failed: No adequate face data.",
                        text_color="red",
                    )
                    return
                ...
                self.results_text.insert(
                    ctk.END, f"Registered: {name} (ID: {person_id})\n"
                )  # Use ctk.END for appending
                self.name_entry.delete(
                    0, ctk.END
                )  # Clear name entry after successful registration
                self.status_label.configure(
                    text=f"Registered '{name}' successfully!", text_color="green"
                )
                self.root.after(
                    0, self._load_persons_to_gui
                )  # Refresh person list after registration
            except Exception as e:
                logging.error(
                    f"Registration failed for {name}: {e}", exc_info=True
                )  # Log full traceback
                messagebox.showerror(
                    "Registration Error",
                ...
                self.status_label.configure(
                    text="Registration failed due to an error.", text_color="red"
                )
            finally:
    ...
    def login_person(self):
        ...
        def login_thread_target():
            ...
            try:
                ...
                self._set_interaction_buttons_state("disabled")
                self.status_label.configure(
                    text="Logging in: Capturing face samples...", text_color="yellow"
                )
                self.root.update_idletasks()  # Force GUI update to show status
                ...
                if embedding is None:
                    ...
                    self.status_label.configure(
                        text="Login failed: No adequate face data.", text_color="red"
                    )
                    return

                person_id, person_data, min_distance = self.person_db.find_match(
                    embedding, self.threshold
                )

                # Update the displayed minimum distance
                self.min_distance_label.configure(
                    text=f"Last Match Distance: {min_distance:.5f}"
                )
                ...
                if person_data:
                    ...
                    self.status_label.configure(
                        text=f"Login: {person_data['name']} - {access_status}!",
                    ...
                    self.status_label.configure(
                        text="Login failed: No match found.", text_color="red"
                    )
                ...
                messagebox.showerror(
                    "Login Error", f"An unexpected error occurred during login: {e}"
                )
                self.status_label.configure(
                    text="Login failed due to an error.", text_color="red"
                )
            finally:
                self.processing = False
                self._set_interaction_buttons_state("normal")
    ...
    def _on_inner_frame_configure(self, event=None):
        ...
        self.person_list_canvas.configure(
            scrollregion=self.person_list_canvas.bbox("all")
        )
    ...
    def _load_persons_to_gui(self):
        ...
        label_text_color = ctk.ThemeManager.theme["CTkLabel"]["text_color"][mode_index]

        if not persons:
            label = ctk.CTkLabel(
                self.person_list_inner_frame,
                text="No registered persons yet.",
                font=("Arial", 12),
        ...
        self.person_list_inner_frame.grid_columnconfigure(
            0, weight=1, minsize=100
        )  # ID
        self.person_list_inner_frame.grid_columnconfigure(
            1, weight=2, minsize=150
        ...
        for i, person in enumerate(persons):
            row_frame = ctk.CTkFrame(
                self.person_list_inner_frame, fg_color="transparent"
            )
            ...
            created_at_formatted = datetime.fromisoformat(
                person["created_at"]
            ).strftime("%Y-%m-%d %H:%M")
            ...
            ctk.CTkLabel(
                ...
                font=("Arial", 12),
                text_color=label_text_color,
            ).grid(row=0, column=1, padx=2, pady=1, sticky="w")
            ctk.CTkLabel(
                row_frame,
                text=allowed_status,
                font=("Arial", 12),
                text_color=label_text_color,
            ).grid(row=0, column=2, padx=2, pady=1, sticky="w")
            ctk.CTkLabel(
                row_frame,
                text=created_at_formatted,
                font=("Arial", 12),
                text_color=label_text_color,
            ).grid(row=0, column=3, padx=2, pady=1, sticky="w")

            delete_btn = ctk.CTkButton(
                row_frame,
                text="Delete",
                command=lambda p_id=person_id: self._handle_delete_person_from_list(
                    p_id
                ),
                width=80,
                height=25,
    ...
    def _handle_delete_person_from_list(self, person_id):
        ...
        if messagebox.askyesno(
            "Confirm Deletion",
            f"Are you sure you want to delete person with ID:\n{person_id}?",
        ):
            if self.person_db.delete_person(person_id):
                messagebox.showinfo(
                    "Success", f"Person with ID '{person_id}' deleted successfully."
                )
                logging.info(f"Deleted person with ID: {person_id}")
                self.status_label.configure(
                    text=f"Deleted person '{person_id}' successfully.",
                    text_color="green",
                )
                self._load_persons_to_gui()  # Refresh list
            else:
                messagebox.showerror(
                    "Error", f"Person with ID '{person_id}' not found in database."
                )
                logging.warning(
                    f"Attempted to delete non-existent person ID: {person_id}"
                )
                self.status_label.configure(
                    text=f"Error: Person '{person_id}' not found.", text_color="red"
                )
    ...
    def _set_interaction_buttons_state(self, state):
        ...
        self.ip_cam_url_entry.configure(
            state=state if self.camera_type_var.get() == "IP Cam" else "disabled"
        )
    ...
    def exit_program(self):
        ...
        if messagebox.askokcancel("Exit", "Are you sure you want to exit the program?"):
            ...
            if self.capture_thread and self.capture_thread.is_alive():
                ...
                if self.capture_thread.is_alive():
                    logging.warning(
                        "Capture thread did not terminate cleanly. It might be stuck."
                    )
...
"""
If this script is run directly, it will create an instance of the `FaceRecognitionApp`
class and start the main application event loop.

The following command line arguments are accepted:

* `--database-path <path>`: specifies the path to the SQLite database file
    used to store registered person data.
* `--initial-camera-index <int>`: specifies the index of the built-in camera
    to use for capturing faces. The default is 0 (the default camera).
* `--initial-ip-cam-url <url>`: specifies the URL of the IP camera to use
    for capturing faces, if the `--initial-camera-type` argument is `IP Cam`.
    The default is `http://192.168.1.5/cam-hi.jpg`.
* `--initial-camera-type <str>`: specifies the type of camera to use for
    capturing faces. The default is `Built-in Cam`. Accepted values are
    `Built-in Cam` and `IP Cam`.
"""
if __name__ == "__main__":
    ...
    embedding_predictor = EmbeddingPredictor(
        model_path="models/resarksgd/resarksgdaug94.pth", device="cpu"
    )
    ...
    custom_cnn_model_path = (
        "models/bbox_models/v5/bbox_v5_randomly_augmented_epoch_3.pth"
    )
    ...
    app = FaceRecognitionApp(
        ...
        threshold=0.5,  # Cosine distance threshold. Lower values require higher similarity (e.g., 0.1 is very strict, 0.4 is more lenient).
